...
`PhpRefactor` is the high-level orchestration package for composed PHP refactoring workflows. It orchestrates `php-rename` and `php-retype` operations in one global transaction without duplicating semantic resolution, AST mutation, or source-writing responsibilities.
